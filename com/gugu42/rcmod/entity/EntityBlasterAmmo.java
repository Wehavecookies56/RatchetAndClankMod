...
public class EntityBlasterAmmo extends Entity {
	...
	public EntityBlasterAmmo(World world, double d, double d1, double d2) {
		this(world);
		setPosition(d, d1, d2);
		yOffset = 0.0F;
	...
	public EntityBlasterAmmo(World world, EntityLivingBase entity,
		...
		setLocationAndAngles(entity.posX + d,
				entity.posY + (double) entity.getEyeHeight(), entity.posZ + d1,
				entity.rotationYaw + f3, entity.rotationPitch + f4);
		...
		setBulletHeading(motionX, motionY, motionZ, 2f, 1.0F);
		double d2 = 0.0D;
		double d3 = 0.0D;
		double d4 = 0.0D;
		...
		if (entity.ridingEntity != null) {
			...
			d3 = entity.ridingEntity.onGround ? 0.0D
	...
	public void onUpdate() {
		...
		Vec3 vec3d = this.worldObj.getWorldVec3Pool().getVecFromPool(posX,
				posY, posZ);
		...
		vec3d = this.worldObj.getWorldVec3Pool().getVecFromPool(posX, posY,
				posZ);
		...
		for (int j = 0; j < list.size(); j++) {
			...
			if (!entity1.canBeCollidedWith()
					|| (entity1 == owner || owner != null
							&& entity1 == owner.ridingEntity || owner != null
							&& entity1 == owner.riddenByEntity)
					&& timeInAir < 5 || minecraftServerDotJavaSpawned) {
				continue;
			}
		...
		if (movingobjectposition != null && !worldObj.isRemote) {
			int k = worldObj.getBlockId(movingobjectposition.blockX,
			...
			if (movingobjectposition.entityHit != null
				...
				if (movingobjectposition.entityHit != null) {
					movingobjectposition.entityHit.attackEntityFrom(
							DamageSource.causeThrownDamage(this, owner), 2f);
				} else {
					...
					float f2 = MathHelper.sqrt_double(motionX * motionX
		...
		for (rotationPitch = (float) ((Math.atan2(motionY, f1) * 180D) / Math.PI); rotationPitch
		...
		if (handleWaterMovement()) {
			for (int i1 = 0; i1 < 4; i1++) {
				...
				worldObj.spawnParticle("bubble", posX - motionX * (double) f6,
						posY - motionY * (double) f6, posZ - motionZ
								* (double) f6, motionX, motionY, motionZ);
			}
