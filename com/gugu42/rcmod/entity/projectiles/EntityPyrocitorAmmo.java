...
public class EntityPyrocitorAmmo extends EntityThrowable {
	...
	public EntityPyrocitorAmmo(World par1World, EntityPlayer par2EntityLiving, float random) 
	{
		super(par1World, par2EntityLiving);
        ...
        this.setThrowableHeading(this.motionX, this.motionY, this.motionZ, this.func_70182_d()+(rand.nextFloat()*5f), random);
        this.startX = posX;
        this.startY = posY;
        this.startZ = posZ;
	}
	...
	public void onUpdate() {
		...
		if(Math.sqrt(Math.pow(posX-startX, 2)+Math.pow(posY-startY, 2)+Math.pow(posZ-startZ, 2)) > MAX_DISTANCE*2 && !worldObj.isRemote)
		{
			setDead();
		}
	}
	...
	protected void onImpact(MovingObjectPosition movingobjectposition) {
		if (movingobjectposition.entityHit != null && movingobjectposition.entityHit != this && !(movingobjectposition.entityHit instanceof EntityPyrocitorAmmo)) {
				byte b0 = 6;
