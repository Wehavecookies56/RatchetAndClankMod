...
public class PacketShipPlatform extends AbstractPacket {
	...
	public void handleClientSide(EntityPlayer player) {
		System.out.println("[CLIENT] Received beacon data, processing ...");
		String[] infos = saveData.split(";");
		if (infos.length == 6) {
			System.out.println("[CLIENT] Enough args ...");
			...
			if (te instanceof TileEntityShipPlatform) {
				System.out.println("[CLIENT] Tile Entity found ...");
				...
				String wpData = infos[0] + " " + infos[3] + " " + infos[4] + " " + infos[5] + " " + infos[1] + " " + infos[2].toLowerCase();
				if (ShipSystem.isNameTaken(infos[0])) {
					System.out.println("[CLIENT] Beacon name already taken.");
				} else {
					ShipWaypoint wp = new ShipWaypoint(wpData);
					if(wp != null){
						ShipSystem.addWaypoint(wp);
						System.out.println("[CLIENT] Beacon data successfully processed.");
					} else {
						System.out.println("[CLIENT] Error while processing data.");
					}
				}
			}
		}
