...
public class ItemSuckCannon extends ItemRcWeap {
	...
	public void onUpdate(ItemStack stack, World w, Entity owner, int i,
			boolean held) {
				...
				if (angle < 0.25 * Math.PI && angle > -0.25 * Math.PI) {
					if (entity.getDistanceToEntity(owner) >= 2.5) {
						...
						customKnockBack(entity, 0, b.xCoord * suckingPower,
								b.yCoord * suckingPower, b.zCoord
										* suckingPower);
					} else {
						entity.setDead();
						...
						if (!world.isRemote) {
							ExtendedPropsSuckCannon props = ExtendedPropsSuckCannon
									.get(owner);
...
>>>>>>> origin/master
	...
	public void customKnockBack(EntityLivingBase par1Entity, float par2,
		...
		if (par1Entity.worldObj.rand.nextDouble() >= par1Entity
